...
![A Microsoft Surface (SUR40) seen here with three distinct TUIO Objects/Tokens. Each of those can be uniquely identified. The top screen shows the objects as they are recognized.](./figures/surface.jpeg){#fig:surface}

In essence, the mentioned controller interface is in charge of recognizing user input. An application needs to receive this information, and this is the role of TUIO. It standardizes a communication protocol by defining different message types and the way they are transmitted from a TUIO Server to a TUIO Client. An application that acts as a TUIO Client can thus receive user input information from any source that can transmit this information acting as a TUIO Server. Figure {@fig:surface} shows a Microsoft Surface multitouch table, paired with an additional screen; the device recognizes user input -- in this case several tangible objects. The device acts as both a Server and a Client in this example. TUIO was built on top of the Open Sound Control protocol, which means that this information is transmitted over a network. This allows applications to receive user input from different physical devices, if necessary. As stated in the specification [@tuio1spec]:
...
The two protocols are not backwards compatible[@tuio2spec]; other than the types in some cases having different capability, the messages are also structured differently. However in general, Pointers refer to the same input as Cursors of TUIO 1 (e.g. fingers) and the same is true for Tokens and Objects.
